...
export const createEventReport = async (req, res) => {
  try {
    ...
    // Sanitize or generate excerpt
    const cleanExcerpt =
      excerpt && excerpt.trim()
        ? sanitizeHtml(excerpt, { allowedTags: [], allowedAttributes: {} }) // plain text
...
export const getDeletedEventReports = async (req, res) => {
  try {
    const deletedReports = await EventReport.find({ deleted: true }).populate(
      "event",
      "title startDate"
    );
